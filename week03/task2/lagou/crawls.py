...
class ShedulerThread(Thread):
    def __init__(self, 
        ...
        logger.info(f'创建生产url线程<{self.name}>')
    
    def run(self):
        # tools.lock_
        logger.info(f'启动生产url线程<{self.name}>')
        for urls_info in self.urls_info:
            ...
            try:
                ...
                break
        # tools.lock_
        logger.info(f'生产url线程线程<{self.name}>结束')
...
class RequestThread(Thread):
    def __init__(self,
        ...
        logger.info(f'创建页面请求线程<{self.name}>')
...
class ParserThread(Thread):
    def __init__(self,
        ...
        logger.info(f'创建页面解析线程<{self.name}>')

    def run(self):
        # tools.lock_
        logger.info(f'启动页面解析线程<{self.name}>')
        # 当收集数据未达到目标值时，执行循环
        while True:
            try:
        ...
        # tools.lock_
        logger.info(f'页面解析线程<{self.name}>结束')
    ...
    def parser(self, content):
        ...
        # tools.lock_
        logger.info(f'<{self.name}>: \n-'
                    f' 解析页面 {unquote_plus(content.request.url)} ')
